...
// Get browser speech support and current speech state for debug pages
export function getSpeechStatus() {
  const hasWindow = typeof window !== 'undefined'
  const speechSynthesis = hasWindow ? window.speechSynthesis : null

  return {
    supported: Boolean(speechSynthesis && hasWindow && 'SpeechSynthesisUtterance' in window),
    voicesLoaded: speechSynthesis ? speechSynthesis.getVoices().length : 0,
    browserSpeaking: speechSynthesis ? speechSynthesis.speaking : false,
    browserPaused: speechSynthesis ? speechSynthesis.paused : false,
    browserPending: speechSynthesis ? speechSynthesis.pending : false,
