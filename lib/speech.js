...
export function initSynth() {
  ...
  // Force Chrome to load voices (Chrome requires this)
  try {
    // Chrome workaround: force voice loading
    if (window.speechSynthesis.getVoices().length === 0) {
      console.log('🔧 Chrome detected - forcing voice loading...')
      window.speechSynthesis.getVoices()
    }
  } catch (e) {
    console.log('⚠️ Chrome voice loading workaround failed:', e)
  }
  ...
  try {
    ...
    if (voices && voices.length > 0) {
      ...
      setTimeout(() => {
        if (!detectionComplete) {
          console.log('⏰ Timeout waiting for voices, forcing re-initialization...')
          // Force Chrome to load voices again
          try {
            window.speechSynthesis.getVoices()
            const voices = window.speechSynthesis.getVoices()
            if (voices && voices.length > 0) {
              console.log(`✅ Voices loaded after timeout: ${voices.length} voices`)
              synthesizerSupported = true
              voicesLoaded = true
            } else {
              console.log('❌ Still no voices after timeout')
              synthesizerSupported = false
            }
          } catch (e) {
            console.log('❌ Error during timeout voice loading:', e)
            synthesizerSupported = false
          }
          detectionComplete = true
          window.speechSynthesis.removeEventListener('voiceschanged', handleVoicesChanged)
...
export async function speakText(text, onComplete, options = {}) {
  ...
  // If voices aren't loaded yet, try to force load them
  if (!voicesLoaded && typeof window !== 'undefined' && 'speechSynthesis' in window) {
    console.log('🔄 Voices not loaded, attempting to force load...')
    try {
      window.speechSynthesis.getVoices()
      ...
      const voices = window.speechSynthesis.getVoices()
      if (voices && voices.length > 0) {
        console.log(`✅ Voices loaded after force load: ${voices.length} voices`)
        voicesLoaded = true
        synthesizerSupported = true
      }
    } catch (e) {
      console.log('⚠️ Force voice loading failed:', e)
    }
  }
